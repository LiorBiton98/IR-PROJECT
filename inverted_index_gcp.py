...
class MultiFileReader:
    ...
    def read(self, locs, n_bytes):
        ...
        for f_name, offset in locs:
            f_name = f_name.replace('\\', '/')  # Replace backslashes with forward slashes
            f_path = str(self._base_dir / f_name) if self._bucket is None else f_name
            if f_path not in self._open_files:
                self._open_files[f_path] = _open(f_path, 'rb', self._bucket)
...
class InvertedIndex:  
    ...
    def read_index(base_dir, name, bucket_name=None):
        file_path = f"{base_dir}/{name}.pkl"
        bucket = None if bucket_name is None else get_bucket(bucket_name)
        with _open(file_path, 'rb', bucket) as f:
